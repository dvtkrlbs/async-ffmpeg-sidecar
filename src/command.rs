...
/// documentation: <https://ffmpeg.org/ffmpeg.html>. Refer there for the exhaustive
...
impl FfmpegCommand {
    ...
    pub fn input<S: AsRef<str>>(&mut self, path: S) -> &mut Self {
        self.arg("-i");
    ...
    ///
    /// Since this is the last argument in the command and no `-` flag
    ...
    pub fn output<S: AsRef<str>>(&mut self, path: S) -> &mut Self {
        self.arg(path.as_ref());
        self
    }
    ...
    ///
    /// Identical to `arg` in [`tokio::process::Command`].
    ...
    ///
    /// Identical to `args` in [`tokio::process::Command`].
    ...
    ///
    /// Identical to `get_args` in [`tokio::process::Command`].
    ...
    pub fn print_command(&mut self) -> &mut Self {
        let program = self.inner.as_std().get_program().to_str();
        let args = self
            .get_args()
            .filter_map(|s| {
                s.to_str().map(|s| {
    ...
    //
    //     }
    //     self.as_inner_mut().win();
    //     self
    // }

    //// Constructors
    pub fn new() -> Self {
        Self::new_with_path("ffmpeg")
    }

    pub fn new_with_path<S: AsRef<OsStr>>(path: S) -> Self {
        let mut inner = Command::new(&path);
    ...
    /// Escape hatch to mutably access to the inner `Command`.
